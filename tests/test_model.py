...
class TestAudioFeatureExtractor:
    ...
    def test_extract_features_from_tensor(self, device, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "mfcc_extraction_tensor", iterations=100, extra_info={"device": str(device)}
        ):
            for _ in range(100):
    ...
    def test_load_audio_bytes(self, sample_audio, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark("load_audio_bytes", iterations=100):
            for _ in range(100):
...
class TestWakeWordCNN:
    ...
    def test_model_inference_latency(self, device, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "cnn_single_inference", iterations=1000, extra_info={"device": str(device)}
        ):
            with torch.no_grad():
                for _ in range(1000):
                    output = model(x)
...
class TestNegativeSampleGenerator:
    ...
    def test_generate_noise_samples(self, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "generate_noise_samples", extra_info={"num_samples": num_samples}
        ):
            samples = generator.generate_noise_samples(count=num_samples, duration=duration)
...
class TestWakeWordTrainer:
    ...
    def test_train_model_quick(self, device, benchmark_tracker):
        ...
        def progress_cb(epoch, total, metrics):
            print(
                f"  Epoch {epoch}/{total}: loss={metrics['train_loss']:.4f}, acc={metrics['train_acc']:.4f}"
            )
