...
class TestEndToEndTraining:
    ...
    def test_synthetic_training_pipeline(self, device, temp_dir, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "e2e_synthetic_positive_generation", extra_info={"num_samples": num_samples}
        ):
        ...
        def progress_cb(epoch, total, metrics):
            if epoch % 2 == 0:
                print(
                    f"  Epoch {epoch}/{total}: loss={metrics['train_loss']:.4f}, val_acc={metrics['val_acc']:.4f}"
                )

        with benchmark_tracker.benchmark(
            "e2e_model_training", extra_info={"device": str(device), "epochs": 10}
        ):
        ...
        print(
            f"  Final: train_loss={history['train_loss'][-1]:.4f}, val_acc={history['val_acc'][-1]:.4f}"
        )
        ...
        # Test on noise
        noise_bytes = create_audio_bytes(
            np.random.randn(num_audio_samples).astype(np.float32) * 0.1
        )
...
class TestModelAccuracy:
    ...
    def test_model_accuracy_on_distinguishable_classes(self, device, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "accuracy_training", extra_info={"samples": num_samples * 2, "epochs": 20}
        ):
            history = trainer.train(
                positive_samples=positive_samples,
                negative_samples=negative_samples,
...
class TestGPUPerformanceIntegration:
    ...
    def test_training_gpu_utilization(self, device, temp_dir, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "gpu_training_full", extra_info={"samples": num_samples * 2, "epochs": 10}
        ):
            history = trainer.train(
                positive_samples=positive_samples,
                negative_samples=negative_samples,
    ...
    def test_batch_inference_throughput(self, device, benchmark_tracker):
        ...
        for batch_size in batch_sizes:
            ...
            print(
                f"  Batch {batch_size:3d}: {throughput:>10,.0f} samples/sec ({elapsed/iterations*1000:.2f} ms/batch)"
            )
