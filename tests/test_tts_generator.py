...
class TestAudioAugmenter:
    ...
    def test_augment_sample_batch(self, benchmark_tracker):
        ...
        all_augmented = []
        with benchmark_tracker.benchmark("augment_batch", extra_info={"num_samples": num_samples}):
...
class TestSampleGenerator:
    ...
    async def test_generate_multiple_samples(self, generator, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "generate_samples_batch", extra_info={"target_count": target_count}
        ):
            samples = await generator.generate_samples(
                word="jarvis",
                target_count=target_count,
                progress_callback=lambda cur, tot, stage: print(
                    f"  Progress: {cur}/{tot} - {stage}"
                ),
            )
    ...
    async def test_save_samples(self, generator, temp_dir):
        ...
        samples = await generator.generate_samples(word="test", target_count=5)
...
class TestTTSEngineCoverage:
    ...
    async def test_engine_diversity(self, generator, benchmark_tracker):
        ...
        samples = await generator.generate_samples(word="hey computer", target_count=20)
        ...
        print(
            f"Samples per engine: {dict((e.value, sum(1 for s in samples if s.engine == e)) for e in engines_used)}"
        )
...
class TestAugmentationDataset:
    ...
    async def test_augment_dataset(self, temp_dir, benchmark_tracker):
        ...
        with benchmark_tracker.benchmark(
            "augment_dataset", extra_info={"base_samples": len(samples)}
        ):
