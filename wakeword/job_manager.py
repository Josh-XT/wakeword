...
class JobManager:
    ...
    def _load_jobs(self):
        ...
        for job_file in self.jobs_dir.glob("*.json"):
            try:
                ...
                logger.info(f"Loaded job {job.job_id} for word '{job.word}' (status: {job.status.value})")
    ...
    async def _run_training(
        ...
        try:
            ...
            from .model import (
                ModelConfig, WakeWordTrainer, NegativeSampleGenerator
            )
            ...
            generator = SampleGenerator(
                ...
                chatterbox_voices_dir=settings.voices_dir if settings.tts_chatterbox_enabled else None,
            )
